...
const gatherData = (): AppData => {
  ...
  const charts = JSON.parse(localStorage.getItem('poker-charts') || '[]'); // Retrieve charts
...
const applyData = (data: AppData) => {
  ...
  localStorage.setItem('poker-charts', JSON.stringify(data.charts || [])); // Apply charts data
